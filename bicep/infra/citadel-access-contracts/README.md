...
## 🛡️ Business Continuity & Resiliency (optional)

A single access contract can optionally be mirrored across **additional APIM gateways, Key Vaults, and Foundry instances** so it keeps working if the primary instance or region becomes unavailable. The subscription key generated by the primary gateway is **reused across all gateways**, so client applications use the **same `api-key`** regardless of which gateway serves the request (ideal behind a global load balancer such as Azure Front Door or Traffic Manager).

- 🔁 **Same key everywhere**: primary gateway generates the key; additional gateways reuse it verbatim.
- 🌐 **Optional global endpoint**: point Key Vault/Foundry at a global load balancer URL (like Azure Traffic Manager endpoint).
- 🧩 **Opt-in per instance**: add secondary Key Vaults / Foundry projects with configurable endpoint source.
- 📦 **Staged deployment**: primary first → additional gateways → Key Vaults & Foundry.
...
| `globalGatewayUrl` | string | ❌ | Global endpoint (e.g. Front Door) used for all KV/Foundry endpoints (default: `''` = primary gateway). See [Resiliency Guide](./access-contract-resiliency-guide.md) | `https://ai-gateway.contoso.com` |
