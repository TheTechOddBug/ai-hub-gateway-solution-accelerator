...
- **Idempotent redeploys for key rotation.** Rotating a subscription key is "rotate on primary → re-run the same deployment" — the pipeline re-applies the shared key to every region. Automate this so all gateways stay in sync (see [Key rotation](../bicep/infra/citadel-access-contracts/access-contract-resiliency-guide.md#-key-rotation)).
